...
collection：查询集合
limit:条目数
sortobj:{key:1 or -1}key是排序字节
*/
exports.findlimit = (dbname,collectionname,obj,sortobj,limit,callback)=>{
  Client.connect(contstr.url,(err,db)=>{
    if(err) return callback(err);
    const collection = db.db(dbname).collection(collectionname);
    collection.find(obj).sort(sortobj).limit(limit).toArray((err,result) => {
      if(err) return callback(err);
      callback(result);
    });
  });
